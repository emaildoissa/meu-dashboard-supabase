...
import { 
  Box, Typography, Paper, CircularProgress, useTheme, useMediaQuery, 
  Alert, Snackbar, Chip, Button, Tooltip 
} from '@mui/material';
import VpnKeyIcon from '@mui/icons-material/VpnKey';
import SmartToyIcon from '@mui/icons-material/SmartToy';

function ChatView() { // Não recebe mais props, ele gerencia tudo internamente
  ...
  // Busca as mensagens e escuta em tempo real quando uma conversa é selecionada
  useEffect(() => {
    ...
    const channel = supabase.channel(`chat_${activeConversation.conversation_id}`)
      .on('postgres_changes', 
        { event: 'INSERT', schema: 'public', table: 'chat_messages', filter: `phone=eq.${activeConversation.conversation_id}` },
        (payload) => {
          ...
          setMessages(currentMessages => [...currentMessages, ...formatted]);
        }
      ).subscribe();
  ...
  return (
    <>
      <Box sx={{ height: '100%', display: 'flex', flexDirection: isMobile ? 'column' : 'row' }}>
        <Box sx={{ width: isMobile ? '100%' : '320px', height: isMobile ? '40%' : '100%', flexShrink: 0, borderRight: { md: '1px solid #e0e0e0' } }}>
            ...
            <Paper elevation={0} sx={{ height: '100%', display: 'flex', flexDirection: 'column', borderRadius: 0 }}>
              <Box sx={{ p: 2, borderBottom: '1px solid #e0e0e0', backgroundColor: '#fafafa', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <Box>
                  ...
                  <Chip label={isHumanControl ? 'Controle Humano' : 'Controlado pela IA'} color={isHumanControl ? 'warning' : 'success'} size="small" icon={isHumanControl ? <VpnKeyIcon /> : <SmartToyIcon />} />
                </Box>
                <Tooltip title={isHumanControl ? 'Devolver controle para a IA' : 'Assumir controle da conversa'}><Button variant="contained" size="small" color={isHumanControl ? 'success' : 'warning'} onClick={handleStatusToggle}>{isHumanControl ? 'Devolver ao Robô' : 'Assumir Controle'}</Button></Tooltip>
              </Box>
              {loadingMessages ? (<Box sx={{ flex: 1, display: 'flex', justifyContent: 'center', alignItems: 'center' }}><CircularProgress /></Box>) : (<ChatViewer messages={messages} />)}
      ...
      <Snackbar open={notification.open} autoHideDuration={4000} onClose={() => setNotification(prev => ({ ...prev, open: false }))} anchorOrigin={{ vertical: 'bottom', horizontal: 'center' }}>
        <Alert onClose={() => setNotification(prev => ({ ...prev, open: false }))} severity={notification.severity} variant="filled" sx={{ width: '100%' }}>{notification.message}</Alert>
      </Snackbar>
    </>
  );
}
